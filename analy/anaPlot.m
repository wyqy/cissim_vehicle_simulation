...
% 则结果一共有n行, sizes列.
    ...
    % 参数
    cellsize = length(data_ori);
    datasize = size(data_ori{1}, 2);
    % 检查
    ...
    % 绘图
    figure;
    tl = tiledlayout(cellsize, datasize);
    tl.Title.String = titletext;
    % 子图
    for iter_row = 1:cellsize
        for iter_col = 1:datasize
            nexttile((iter_row-1)*datasize+iter_col);
